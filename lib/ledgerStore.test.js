...
async function run() {
  ...
  await test('logCompletedSession accepts a pre-computed duration directly (no active-session tracking)', async () => {
    const actions = makeLedgerActions(createMemoryStore());
    const result = await actions.logCompletedSession('emma', '2026-07-13T16:00:00.000Z', 24, 3200);
    assert.strictEqual(result.minutesRead, 24);
    assert.strictEqual(result.wordsPerMinute, 3200 / 24);
    const balance = await actions.getBalance('emma', '2026-07-13T16:00:00.000Z');
    assert.strictEqual(balance.bonusHoursEarned, 24 / 30);
  });

  await test('logCompletedSession works without a word count too', async () => {
    const actions = makeLedgerActions(createMemoryStore());
    const result = await actions.logCompletedSession('jack', '2026-07-13T16:00:00.000Z', 30);
    assert.strictEqual(result.hoursEarned, 1);
    assert.strictEqual(result.wordsPerMinute, null);
  });

  await test('logCompletedSession rejects negative or non-numeric minutes', async () => {
    const actions = makeLedgerActions(createMemoryStore());
    await assert.rejects(
      () => actions.logCompletedSession('jack', '2026-07-13T16:00:00.000Z', -5),
      { code: 'INVALID_MINUTES' }
    );
    await assert.rejects(
      () => actions.logCompletedSession('jack', '2026-07-13T16:00:00.000Z', 'not a number'),
      { code: 'INVALID_MINUTES' }
    );
  });

  await test('attachWordCount fills in real WPM after the fact, via sessionId', async () => {
    const store = createMemoryStore();
    const actions = makeLedgerActions(store);
    const logResult = await actions.logCompletedSession(
      'emma',
    ...
    // Word count isn't known yet at log time — transcription is async
    assert.strictEqual(logResult.wordsPerMinute, null);

    const wpm = await actions.attachWordCount('emma', logResult.weekId, 'session-abc', 2400);
    assert.strictEqual(wpm, 120); // 2400 words / 20 min

    // Balance/hoursEarned is unaffected — it was already locked in by minutes
    const balance = await actions.getBalance('emma', '2026-07-13T16:00:00.000Z');
    assert.strictEqual(balance.bonusHoursEarned, 20 / 30);
  });

  await test('attachWordCount returns null for an unrecognized sessionId', async () => {
    const store = createMemoryStore();
    const actions = makeLedgerActions(store);
    await actions.logCompletedSession('emma', '2026-07-13T16:00:00.000Z', 20, null, 'real-session');
    const weekId = require('./ledger').getWeekId('2026-07-13T16:00:00.000Z');
    const result = await actions.attachWordCount('emma', weekId, 'does-not-exist', 500);
