...
function makeLedgerActions(store) {
   ...
   */
  async function logCompletedSession(childId, nowIso, minutesRead, wordsRead = null) {
    ...
    await store.saveWeek(childId, weekId, scratch[childId][weekId]);
    return { minutesRead, hoursEarned, wordsPerMinute, weekId };
  }

  return { startReading, stopReading, getBalance, spendBonus, logCompletedSession };
}
